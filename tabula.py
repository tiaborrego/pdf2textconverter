from tabula import wrapper, read_pdf, convert_into, convert_into_by_batch
import pandas as pd
import numpy as np
import os
...
while True:
    directory = input("/Users/tiaborrego/Desktop/TCM_final/pdfs")
    print("You entered input path " + directory)
    if directory:
        break
while True:
    output_path = input("/Users/tiaborrego/Desktop/TCM_final/tables")
    print("You entered output path " + output_path)
    if output_path:
        break
while True:
    output_file = input(directory + "/" + ".xlsx")
